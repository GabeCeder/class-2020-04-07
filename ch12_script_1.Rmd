...
x %>% 
  summarize(cor(faculty, earnings))

x %>% 
  summarize(cor(sat, earnings))
...
model_a <- lm(earnings ~ sat + faculty, data = x)
...
- if you have an average sat score of 0 and average faculty income of 0, then you expect earnings to be -2590.98. 
- for each 1 point increase in the average sat score, you expect to see a 24.48 dollar increase in earnings.
- for each 1 dollar increase in average faculty salary, you expect to see a 0.39 dollar increase in earnings. 
...
```{r}
model_b <- lm(earnings ~ public + sat + faculty + price, data = x) %>% 
  tidy(conf.int = TRUE)
...
RCM we don't know what the counterfactual outcome is for a given school. Given our regression model, we would expect a 100 point decrease in average SAT scores to reduce earnings by 1245 dollars. 
...
a <- tibble(faculty = 50000, sat = 1400)

predict(model_a, newdata = a)
